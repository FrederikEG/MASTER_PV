...
ax1.plot(continuous_time,eta_with_bi[common_index],label='Eta w. bi')
...
ax1.text(continuous_time[len(time_index1_date) + len(common_time_index2) + len(common_time_index3) + round((len(common_time_index4))/2)] + 300 , -0.1*eta_lim, str(time_index4_date[0])+ '\n' + time_index4_date[-1], fontsize=8, ha='center', va='center', family='sans-serif', 
         bbox=dict(facecolor='white', edgecolor='black', boxstyle='square,pad=0.5'))
ax1.plot(continuous_time,eta_with_bi_daily_mean[common_index],label='Eta w. bi. daily mean')
ax1.plot(continuous_time,eta_with_bi_mean[common_index],label='Eta w. bi. mean')
...
DC_gen_calc_common_index = (generation_DC1[common_time_index4_grid].sum()/12)*0.001
...
DC_gen_inv_common_index = inverter_DC[common_time_index4_grid].sum()/12
