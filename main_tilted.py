# -*- coding: utf-8 -*-
"""
Created on Wed Mar  6 11:04:03 2024
...
from daily_plots import day_plot, scatter_plot, solar_pos_scat, bar_plots, day_histo_plot
...
"""
model_to_run = {'GHI_sensor':'GHI',                     # 'GHI', 'GHI_SPN1', 'GHI_2nd' (only hour mean), 'GHI_2nd_ws_correct'
                ...
                'inverter_limit' : True,
                'DNI_model': 'dirint'}                      # 'dirint', 'dirindex', 'dirindex_turbidity'                          
"""



model_to_run = {'GHI_sensor':'SPN1',                     # 'GHI', 'SPN1', 'GHI_2nd' (only hour mean)
                'sky_model_inf':'isotropic',            # 'isotropic', 'haydavies'
                'sky_model_simple' : 'isotropic',           # 'isotropic', 'haydavies', 'perez' ......
                'shadow_interpolate' : 'false',          # 'true', 'false'
                'temp_sensor' : 'default',              # 'default', 'weather_station', '2nd weather_station' 
...
POA, solar_position, albedo, GHI, aoi_back, aoi_front, spec_loss, poa_infinite_sheds, albedo_daily, dni, elevation_min = POA(PV_data,
                  installation_data,
                  tz,
...
DC_output, DC_mid_rows, eff_irrad_total, temp_cell, temp_air = DC_generation_temp_select(POA,
                          ...
                          mount_type = model_to_run['mount_type'],
                          inverter_limit = model_to_run['inverter_limit'])
...
POA_simple, poa_no_shadow_front, poa_no_shadow_back, GHI, dni, solar_position, elevation_min = POA_simple(PV_data,
                  installation_data,
                  tz,
                  GHI_sensor= model_to_run['GHI_sensor'],
...
#Uses the temp and wind sensor that works
DC_output_no_shad, DC_mid_rows_no_shad, eff_irrad_total_no_shad, temp_cell_no_shad, temp_air_no_shad = DC_generation_temp_select(POA_simple,
                          PV_data,
                          installation_data,
                          temp_sensor= model_to_run['temp_sensor'],
                          wind_sensor= model_to_run['wind_sensor'],
...
DC_output_ref_cell, DC_mid_rows_ref_cell, eff_irrad_total_ref_cell, temp_cell_ref_cell, temp_air_ref_cell = DC_generation_temp_select(POA_simple,
                          PV_data,
                          installation_data,
                          temp_sensor= model_to_run['temp_sensor'],
                          wind_sensor= model_to_run['wind_sensor'],
...
DC_mid_rows_measure = ((data['TBF PV2 input current (A)']*data['TBF PV2 input voltage (V)']) + (data['TBF PV3 input current (A)']*data['TBF PV3 input voltage (V)']))/1000
...
day_plot('POA back', 
            ...
            value2 = data['Reference Cell Tilted facing down (W.m-2)'],
            days = sun_cloud_days,
            model_to_run = model_to_run,
            model_explain= model_explain,
            solar_position = solar_position['azimuth'],
...
day_plot('POA back - inf and no shadow', 
            'Irradiance',
            value1 = POA['POA fuel_in West'],
            value2 = data['Reference Cell Tilted facing down (W.m-2)'],
...
day=day_plot('POA front - inf and no shadow', 
            'Irradiance',
            value1 = POA['POA fuel_in East'],
            value2 = data['Reference Cell Tilted facing up (W.m-2)'],
            ...
            solar_position = solar_position['azimuth'],
            y_lines = y_lines,
            custom_label= ['POA inf.', 'Ref up', 'POA simple','']
            #custom_yline= 'value2',
...
day_plot('DC generation vertical', 
            'DC Power (kW)',
            value1 = data['INV-1-TBF Total input power (kW)'],
...
day_plot('DC generation Tilted', 
            ...
            model_to_run = model_to_run,
            model_explain= model_explain,
            custom_label=['INV-1-TBF Total input power (kW)', 'p_mp inf.', 'p_mp no_shad', 'p_mp ref cell'],
            zoom = False)
...
day_plot('AC generation vertical', 
            'AC Power (kW)',
            value1 = data['INV-1-TBF Active power (kW)'],
...
day_plot('GHI', 
            ...
            value2 = GHI,
            value3= data[('GHI_SPN1 (W.m-2)')],
            days = sun_cloud_days,
            solar_position= solar_position['azimuth'],
            y_lines= y_lines,
            model_to_run = model_to_run,
            model_explain= model_explain,
            custom_label= ['CMP6', 'GHI from inf', 'SPN1',''])
...
scat_index = scatter_plot('Ref model compare back color inf', 
             y_label = 'measured',
             x_label = 'Modelled',
             modelled_value = POA['POA fuel_in West'], 
             measured_value = data['Reference Cell Tilted facing down (W.m-2)'], 
             #start_date = '2023-04-12 00:00:00', 
...
scat_index = scatter_plot('Ref model compare back color simple', 
             y_label = 'measured',
             x_label = 'Modelled',
...
scat_index = scatter_plot('Ref model compare up color', 
             y_label = 'measured',
             x_label = 'Modelled',
             modelled_value = POA['POA fuel_in East'], 
             measured_value = data['Reference Cell Tilted facing up (W.m-2)'], 
             start_date = '2023-04-12 00:00:00', 
             end_date='2023-05-13 00:00:00',
             solar_position= solar_position,
             interval='false',
             #color_value= data['INV-1-TBF Active power (kW)'] ,
             color_value= solar_position['azimuth'] ,
             y_lim = 1200,
             model_to_run = model_to_run,
             model_explain= model_explain) 
...
RMSE = math.sqrt(np.square(np.subtract(POA['POA fuel_in West'][scat_index],data['Reference Cell Tilted facing down (W.m-2)'][scat_index])).mean())
...
scat_index = scatter_plot('DC generation compare inf', 
             ...
             measured_value = data['INV-1-TBF Total input power (kW)'], 
             start_date = '2023-04-12 00:00:00', 
             end_date='2023-05-13 00:00:00',
             solar_position= solar_position,
             interval='afternoon',
             color_value= solar_position['azimuth'],
             color_interval = 'specific', 
             model_to_run = model_to_run,
             model_explain= model_explain) 
...
scat_index = scatter_plot('DC generation compare ref_cell', 
             ...
             interval='afternoon',
             color_value= solar_position['azimuth'],
             color_interval = 'specific',                  # 'total', 'specific'
             model_to_run = model_to_run,
             model_explain= model_explain,
             y_lim=40) 
...
day_histo_plot(Title = 'POA components for the day inf front', 
               y_label= 'percentage',
               days = sun_cloud_days, 
               model_to_run = model_to_run, 
               model_explain= model_explain,
